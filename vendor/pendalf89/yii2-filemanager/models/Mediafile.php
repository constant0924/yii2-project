...
class Mediafile extends ActiveRecord
{
    ...
    public function saveUploadedFile(array $routes, $rename = false)
    {
        $year = date('Y', time());
        $month = date('m', time());
        $structure = "/$routes[uploadPath]/$year/$month";
    ...
    public function getDefaultThumbUrl($baseUrl = '')
    {
        if ($this->isImage()) {
            ...
            $width = $size[0];
            $height = $size[1];
            return "$baseUrl/$dirname/$filename-{$width}x{$height}.$extension";
        }
    ...
    public function getThumbUrl($alias)
    {
        ...
        if ($alias === 'original') {
            return Yii::$app->params['uploadUrl'].$this->url;
        }
    ...
    public function getImagesList(Module $module)
    {
        ...
        $list[Yii::$app->params['uploadUrl'].$this->url] = Module::t('main', 'Original') . ' ' . $originalImageSize;

        foreach ($thumbs as $alias => $url) {
            $preset = $module->thumbs[$alias];
            $list[Yii::$app->params['uploadUrl'].$url] = $preset['name'] . ' ' . $preset['size'][0] . ' × ' . $preset['size'][1];
        }
        return $list;
    }
